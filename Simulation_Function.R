generate_true_states <- function(M, nsimulations, beta0 = log(c(3,0.5,0.1)), beta1 = c(1,3,1)) {
  
  # Generate covariate for forest density
  Fdens <- runif(n = M, -1, 1)  
  ...
  # Generate true states for each simulation
  ...
  return(list(true_states = true_states, Mean_Omega = colMeans(Omega), Omega = Omega, Fdens = Fdens, beta0 = beta0, beta1 = beta1))
}
  ...
  parameters <- c("beta0[1,1]", "beta0[2,1]","beta0[3,1]","beta1[1,1]", "beta1[2,1]", "beta1[3,1]",
                  "p1",
                  "p21", "p22",
                  "p31", "p32", "p33", "mean.Omega1", "mean.Omega2", "mean.Omega3", "mean.Omega4" 
  )
  ...
  if (show.plot) {
    ...
    plot(Fdens, Omega[, 2], main = "Psi (Occupied without calves) vs Fdens", xlab = "Forest Density", ylab = "Probability")
    plot(Fdens, Omega[, 3], main = "Psi (Occupied with one calf) vs Fdens", xlab = "Forest Density", ylab = "Probability")
    plot(Fdens, Omega[, 4], main = "Psi (Occupied with >1 calves) vs Fdens", xlab = "Forest Density", ylab = "Probability")
  }
...
run_complete_simulation <- function(nsurveysvec, p11, p21, p22, p31, p32, p33,
                                    ...
                                    parallel=TRUE, model_file, M) {

  # Stores the true states for all sites across all simulations
  SimulationParameters <- generate_true_states(M,nsimulation)
